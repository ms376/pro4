...
function fetchDataAndFillTable(currentPage = 1, itemsPerPage = 10) {
   ...
   var pageNumbersContainer = document.getElementById('pageNumbers');
    ...
    const boardsRef = collection(db, 'dongboards');

    
    getDocs(query(collection(db, 'dongboards'), orderBy('num', 'desc')))
        .then((querySnapshot) => {
            ...
            querySnapshot.forEach((doc) => {
                ...
                const tableRow = `<table><tr>
                                     <td>${isHot ? '<span class="new_icon"></span> ' : ''}</td>
                                     <td>${num}</td>
                                     <td>${id}</td>
		 ...
		 titleCells.forEach((cell) => {
		    ...
		    window.location.href = `/dongviewboard?docId=${docId}`; // 클릭 시 viewboard.jsp로 이동합니다.
		    });
		});
