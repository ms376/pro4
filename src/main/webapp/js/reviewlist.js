...
function limitContentLength(content, maxLength) {
    if (content.length <= maxLength) {
        // content의 길이가 maxLength 이하인 경우에는 그대로 반환
        return content;
    } else {
        // content를 maxLength 길이로 자르고 "..."을 추가하여 반환
...
function fetchDataAndFillTable(currentPage = 1, itemsPerPage = 10) {
   ...
   var pageNumbersContainer = document.getElementById('pageNumbers');
    ...
    const boardsRef = collection(db, 'reviewboards');

    
    getDocs(query(collection(db, 'reviewboards'), orderBy('num', 'desc')))
        .then((querySnapshot) => {
            ...
            querySnapshot.forEach((doc) => {
                ...
                const data = doc.data();
                var date = data.date ? data.date.toDate() : null; // Timestamp를 JavaScript Date로 변환
                var formattedDate = date ? formatDate(date) : "날짜 없음";
                const { num, zone, interest, Dname, title, content, count } = data;
                const limitedContent = limitContentLength(content, 20); // 20자로 제한
		 ...
		 titleCells.forEach((cell) => {
		    ...
		    window.location.href = `/reviewviewboard?docId=${docId}`; // 클릭 시 viewboard.jsp로 이동합니다.
		    });
		});
...
function performSearch() {
    ...
    if (searchType === '지역') {
        queryField = 'zone';
    } else if (searchType === '관심 분야') {
        queryField = 'interest';
    } else if (searchType === '동아리 이름') {
	    queryField = 'Dname';
	}
